...
use crate::entities::{Cannon, Player, Projectile, Snake, Spider, Wisp};
use crate::game::{Game, MenuSelection, Scene};
use crate::math::Vec2;
use crate::net::{NetworkSession, NetworkState, PlayerStats, RemotePlayer};
...
impl Renderer {
    ...
    fn render_title(&self, _game: &Game, _network: &NetworkSession) {
        let center_x = (self.width / 2) as f64;

        // Keep only decorative title art in the low-res pass.
        // All title text/UI is drawn in the full-res overlay to avoid duplicate rendering.
    ...
    fn render_game(&self, game: &Game, network: &NetworkSession) {
        ...
        if game.shockwave_timer > 0 {
            ...
            let radius = self
                .world_to_render_size((20.0 + progress as f32 * 70.0) as f64 * CREATURE_SCALE)
                as f64;
        ...
        let (_local_display_name, remote_display_names) = network.display_names_snapshot();
        // Draw remote players
        for (peer_id, remote) in network.remote_players.iter() {
            if remote.alive {
                let display_name = remote_display_names
                    .get(peer_id)
                    .map(String::as_str)
                    .unwrap_or("Player");
                self.render_remote_player(remote, camera, display_name);
            }
        }
    ...
    fn render_shrine(&self, game: &Game) {
        ...
        if shrine_pos.x < min_x - 120.0
            || shrine_pos.x > max_x + 120.0
            || shrine_pos.y < min_y - 120.0
            || shrine_pos.y > max_y + 120.0
        {
            return;
        }
        ...
        self.ctx.fill_rect(
            screen_pos.x as f64 - pillar_offset - pillar_size / 2.0,
            screen_pos.y as f64 - pillar_size / 2.0,
            pillar_size,
            pillar_size,
        );
        self.ctx.fill_rect(
        ...
        self.ctx.fill_rect(
            screen_pos.x as f64 - pillar_size / 2.0,
            screen_pos.y as f64 - pillar_offset - pillar_size / 2.0,
            pillar_size,
            pillar_size,
        );
        self.ctx.fill_rect(
    ...
    fn render_spider(&self, spider: &Spider, frame_count: u32, camera: &Camera) {
        ...
        let flip: f64 = if (frame_count / 5 + spider.id as u32) % 2 == 0 {
            1.0
        } else {
            -1.0
        };
        ...
        let (eye1_x, eye1_y) =
            self.snap_point(x + dir_x * 4.0 + dir_y * 1.5, y + dir_y * 4.0 - dir_x * 1.5);
        let (eye2_x, eye2_y) =
            self.snap_point(x + dir_x * 4.0 - dir_y * 1.5, y + dir_y * 4.0 + dir_x * 1.5);
    ...
    fn render_spider_snapshot(
        ...
        let (x, y) = self.snap_point(screen_pos.x as f64, screen_pos.y as f64);
        let flip: f64 = if (frame_count / 5 + id as u32) % 2 == 0 {
            1.0
        } else {
            -1.0
        };
        ...
        self.ctx
            .fill_rect(eye1_x.round(), eye1_y.round(), eye_size, eye_size);
        self.ctx
            .fill_rect(eye2_x.round(), eye2_y.round(), eye_size, eye_size);
    }
    ...
    fn render_cannon(&self, cannon: &Cannon, camera: &Camera) {
        ...
        self.fill_circle(
            x + cannon.dir.y as f64 * 4.0 * scale,
            y - cannon.dir.x as f64 * 4.0 * scale,
            2.0 * scale,
        );
        self.fill_circle(
            x - cannon.dir.y as f64 * 4.0 * scale,
    ...
    fn render_cannon_snapshot(
        ...
        self.ctx.set_fill_style_str(COLOR_3);
        self.fill_circle(
            x + dir.y as f64 * 4.0 * scale,
            y - dir.x as f64 * 4.0 * scale,
            2.0 * scale,
        );
        self.fill_circle(
            x - dir.y as f64 * 4.0 * scale,
    ...
    fn render_snake(&self, snake: &Snake, frame_count: u32, camera: &Camera) {
        ...
        self.ctx.set_global_alpha(1.0);
        let flip: f64 = if (frame_count / 5 + snake.id as u32) % 2 == 0 {
            1.0
        } else {
            -1.0
        };
    ...
    fn render_guardian(
        &self,
        guardian: &crate::entities::Guardian,
        frame_count: u32,
        camera: &Camera,
    ) {
    ...
    fn draw_tentacle_path(
        &self,
        tentacle: &crate::entities::Tentacle,
        camera: &Camera,
        pulse: f64,
    ) {
        let mut first = true;
    ...
    fn render_player(&self, player: &Player, camera: &Camera) {
        ...
        let body_color = if player.is_phasing() {
            COLOR_ACCENT1
        } else {
            COLOR_ACCENT2
        };
        ...
        if player.is_phasing() {
            ...
            self.draw_circle_outline_masked(
                tail_x,
                tail_y,
                tail_radius,
                render_cx,
        ...
        self.ctx.set_fill_style_str(COLOR_LIGHT);
        let eye1_x = render_cx
            + (player.look_dir.x as f64 * 3.0 + player.look_dir.y as f64 * 1.5)
                * scale
                * self.scale_x;
        let eye1_y = render_cy
            + (player.look_dir.y as f64 * 3.0 - player.look_dir.x as f64 * 1.5)
                * scale
                * self.scale_y;
        let eye2_x = render_cx
            + (player.look_dir.x as f64 * 3.0 - player.look_dir.y as f64 * 1.5)
                * scale
                * self.scale_x;
        let eye2_y = render_cy
            + (player.look_dir.y as f64 * 3.0 + player.look_dir.x as f64 * 1.5)
                * scale
                * self.scale_y;
        ...
        if player.blocking {
            self.ctx.set_fill_style_str(COLOR_LIGHT);
            let s1x = render_cx
                + (player.look_dir.x as f64 * 5.0 - player.look_dir.y as f64 * 7.0)
                    * scale
                    * self.scale_x;
            let s1y = render_cy
                + (player.look_dir.y as f64 * 5.0 + player.look_dir.x as f64 * 7.0)
                    * scale
                    * self.scale_y;
            let e1x = render_cx
                + (player.look_dir.x as f64 * 5.0 + player.look_dir.y as f64 * 7.0)
            ...
            let s2x = render_cx
                + (player.look_dir.x as f64 * 6.0 - player.look_dir.y as f64 * 4.0)
                    * scale
                    * self.scale_x;
            let s2y = render_cy
                + (player.look_dir.y as f64 * 6.0 + player.look_dir.x as f64 * 4.0)
                    * scale
                    * self.scale_y;
            let e2x = render_cx
                + (player.look_dir.x as f64 * 6.0 + player.look_dir.y as f64 * 4.0)
    ...
    fn render_remote_player(&self, remote: &RemotePlayer, camera: &Camera, display_name: &str) {
        ...
        let body_color = if remote.phasing {
            COLOR_ACCENT1
        } else {
            COLOR_REMOTE_PLAYER
        };
        ...
        self.ctx.set_fill_style_str(COLOR_LIGHT);
        let eye1_x = render_cx
            + (remote.look_dir.x as f64 * 3.0 + remote.look_dir.y as f64 * 1.5)
                * scale
                * self.scale_x;
        let eye1_y = render_cy
            + (remote.look_dir.y as f64 * 3.0 - remote.look_dir.x as f64 * 1.5)
                * scale
                * self.scale_y;
        let eye2_x = render_cx
            + (remote.look_dir.x as f64 * 3.0 - remote.look_dir.y as f64 * 1.5)
        ...
        if remote.blocking {
            self.ctx.set_fill_style_str(COLOR_LIGHT);
            let s1x = render_cx
                + (remote.look_dir.x as f64 * 5.0 - remote.look_dir.y as f64 * 7.0)
                    * scale
                    * self.scale_x;
            let s1y = render_cy
                + (remote.look_dir.y as f64 * 5.0 + remote.look_dir.x as f64 * 7.0)
                    * scale
                    * self.scale_y;
            let e1x = render_cx
                + (remote.look_dir.x as f64 * 5.0 + remote.look_dir.y as f64 * 7.0)
            ...
            let s2x = render_cx
                + (remote.look_dir.x as f64 * 6.0 - remote.look_dir.y as f64 * 4.0)
                    * scale
                    * self.scale_x;
            let s2y = render_cy
                + (remote.look_dir.y as f64 * 6.0 + remote.look_dir.x as f64 * 4.0)
                    * scale
                    * self.scale_y;
            let e2x = render_cx
                + (remote.look_dir.x as f64 * 6.0 + remote.look_dir.y as f64 * 4.0)
        ...
        self.ctx.set_text_align("center");
        let _ = self.ctx.fill_text(display_name, cx, cy - 18.0);
    }
    ...
    fn render_network_status(&self, network: &NetworkSession) {
        ...
        let _ = self
            .ctx
            .fill_text(&status_text, (self.width - 10) as f64, 25.0);
    }
    ...
    fn render_title_overlay(&self, game: &Game, network: &NetworkSession) {
        ...
        let _ = self
            .display_ctx
            .fill_text("Name:", center_x - 100.0, name_y);
        ...
        let _ = self
            .display_ctx
            .fill_text("Room:", center_x - 100.0, code_y);
        ...
        self.display_ctx.set_fill_style_str(if join_active {
            COLOR_ACCENT1
        } else {
            COLOR_LIGHT
        });
        ...
        if !game.title_warning.is_empty() {
            self.display_ctx.set_fill_style_str("#ff7a8a");
            self.display_ctx.set_font("11px monospace");
            self.display_ctx.set_text_align("center");
            let _ = self
                .display_ctx
                .fill_text(&game.title_warning, center_x, code_y + 28.0);
        }
    ...
    fn render_game_overlay(&self, game: &Game, network: &NetworkSession) {
        let camera = &game.camera;
        let (local_display_name, remote_display_names) = network.display_names_snapshot();
        ...
        if game.shrine_badge_unlocked {
            let _ = self
                .display_ctx
                .fill_text("Badge: Shrinefinder", 10.0, 85.0);
        }
        ...
        let map_top = if game.mobile_mode || portrait {
            130.0
        } else {
            (self.height as f64) - map_size - map_padding
        };
        if !game.mobile_mode {
            self.display_ctx.set_font("12px monospace");
            self.display_ctx.set_text_align("left");
            let _ = self
                .display_ctx
                .fill_text("M: Map", map_left, map_top - 6.0);
            let _ = self
                .display_ctx
                .fill_text("B: Bind Abilities", map_left, map_top - 22.0);
            let _ = self
                .display_ctx
                .fill_text("4: Payments", map_left, map_top - 36.0);
            let _ = self
                .display_ctx
                .fill_text("F3: Net Debug", map_left, map_top - 50.0);
        }
    ...
    fn render_ability_bar(&self, game: &Game) {
        ...
        for (idx, entry) in entries.iter().enumerate() {
            ...
            let _ = self
                .display_ctx
                .stroke_rect(entry.x, entry.y, entry.w, entry.h);

            self.display_ctx.set_fill_style_str(color);
            let label_y = entry.y + entry.h * 0.55;
            let _ = self
                .display_ctx
                .fill_text(&entry.label, entry.x + entry.w * 0.5, label_y);
    ...
    fn render_network_debug_overlay(&self, network: &NetworkSession) {
        ...
        let _ = self.display_ctx.fill_text(
            &format!(
                "rx:{} up:{} dn:{} bc:{}",
                telemetry.recv_messages,
                telemetry.sent_upstream,
                telemetry.sent_downstream,
                telemetry.sent_broadcast
            ),
            left + 8.0,
            top + 34.0,
        );
        ...
        let _ = self
            .display_ctx
            .fill_text("F3 to hide", left + 8.0, top + 88.0);
    }

    fn render_player_list_overlay(
        &self,
        game: &Game,
        ...
        self.display_ctx
            .stroke_rect(left, top, overlay_w, overlay_h);
        ...
        let _ = self
            .display_ctx
            .fill_text("Players", left + 12.0, top + 24.0);
        ...
        let order_label = if game.player_list_sort_asc {
            "asc"
        } else {
            "desc"
        };
        ...
        for entry in entries.iter().skip(start).take(rows_visible) {
            ...
            let _ = self
                .display_ctx
                .fill_text(&entry.score.to_string(), score_x + 26.0, y);
            let _ = self
                .display_ctx
                .fill_text(&entry.kills.to_string(), kills_x + 20.0, y);
            let _ = self
                .display_ctx
                .fill_text(&entry.deaths.to_string(), deaths_x + 20.0, y);
            let _ = self.display_ctx.fill_text(
                &Self::format_time(entry.time_seconds),
                time_x + 46.0,
                y,
            );
            self.display_ctx.set_text_align("left");
    ...
    fn render_chat_overlay(&self, game: &Game) {
        ...
        let input_height = if game.chat_open {
            line_height + 6.0
        } else {
            0.0
        };
        ...
        self.display_ctx
            .fill_rect(left - 4.0, box_top - 4.0, 360.0, box_height + 8.0);
        ...
        if game.chat_open {
            ...
            let _ = self
                .display_ctx
                .fill_text("Use /mute NAME or /buyname [NAME]", left, helper_y);
            self.display_ctx.set_fill_style_str("#e6efef");
            ...
            if log_lines.is_empty() {
                ...
                let _ = self
                    .display_ctx
                    .fill_text("C: Chat", left, bottom - padding);
            }
        }
    }
    ...
    fn render_mobile_controls(&self, game: &Game) {
        ...
        let _ = self.display_ctx.arc(
            stick_center.x as f64,
            stick_center.y as f64,
            stick_radius,
            0.0,
        ...
        self.display_ctx.begin_path();
        let _ = self.display_ctx.arc(
            attack_center.x as f64,
            attack_center.y as f64,
            action_radius,
            0.0,
            std::f64::consts::PI * 2.0,
        );
        ...
        self.display_ctx.begin_path();
        let _ = self.display_ctx.arc(
            phase_center.x as f64,
            phase_center.y as f64,
            action_radius,
            0.0,
            std::f64::consts::PI * 2.0,
        );
        ...
        self.display_ctx.begin_path();
        let _ = self.display_ctx.arc(
            chat_center.x as f64,
            chat_center.y as f64,
            top_radius,
            0.0,
            std::f64::consts::PI * 2.0,
        );
        ...
        let _ =
            self.display_ctx
                .fill_text("ATT", attack_center.x as f64, attack_center.y as f64 + 4.0);
        let _ =
            self.display_ctx
                .fill_text("PH", phase_center.x as f64, phase_center.y as f64 + 4.0);
        let _ = self
            .display_ctx
            .fill_text("C", chat_center.x as f64, chat_center.y as f64 + 4.0);
        if game.map_open {
            let _ = self.display_ctx.fill_text(
                "+",
                zoom_in_center.x as f64,
                zoom_in_center.y as f64 + 4.0,
            );
            let _ = self.display_ctx.fill_text(
                "-",
                zoom_out_center.x as f64,
                zoom_out_center.y as f64 + 4.0,
            );
        }
    }
    ...
    fn collect_player_entries(&self, game: &Game, network: &NetworkSession) -> Vec<PlayerEntry> {
        ...
        for (peer_id, _remote) in network.remote_players.iter() {
            let stats = network
                .remote_stats
                .get(peer_id)
                .cloned()
    ...
    fn render_minimap(&self, game: &Game, network: &NetworkSession) {
        ...
        self.display_ctx
            .fill_rect(map_left, map_top, map_size, map_size);
        self.display_ctx.set_stroke_style_str("#2a3a3a");
        self.display_ctx.set_line_width(1.0);
        self.display_ctx
            .stroke_rect(map_left, map_top, map_size, map_size);
    ...
    fn render_map_overlay(&self, game: &Game, network: &NetworkSession) {
        ...
        if chunk_px >= 4.0 {
            ...
            for cy in start_cy..=end_cy {
                for cx in start_cx..=end_cx {
                    ...
                    self.display_ctx
                        .fill_rect(x, y, chunk_px.max(1.0), chunk_px.max(1.0));
                }
            }
        }
        ...
        if zoomed_out {
            ...
            for (pos, _kind) in &enemies {
                if pos.x as f64 >= world_min_x
                    && pos.x as f64 <= world_max_x
                    && pos.y as f64 >= world_min_y
                    && pos.y as f64 <= world_max_y
                {
            ...
            let mut players = Vec::new();
            players.push((game.player.pos, local_display_name.clone()));
            for (peer_id, remote) in network.remote_players.iter() {
                if remote.alive {
                    players.push((
                        remote.pos,
            ...
            let mut players = Vec::new();
            players.push((game.player.pos, local_display_name.clone(), COLOR_ACCENT2));
            for (peer_id, remote) in network.remote_players.iter() {
                if remote.alive {
                    players.push((
                        remote.pos,
        ...
        let shrine_pos = crate::game::SHRINE_POS;
        if shrine_pos.x as f64 >= world_min_x
            && shrine_pos.x as f64 <= world_max_x
            && shrine_pos.y as f64 >= world_min_y
            && shrine_pos.y as f64 <= world_max_y
        {
        ...
        if let Some(target) = game.map_target {
            ...
            let valid = !game
                .chunks
                .collides_with_obstacle(target, 4.5 * CREATURE_SCALE as f32);
        ...
        if game.mobile_mode {
            ...
            self.display_ctx
                .fill_rect(button_x, button_y, button_w, button_h);
            self.display_ctx.set_stroke_style_str("#4a5a5a");
            self.display_ctx.set_line_width(1.0);
            self.display_ctx
                .stroke_rect(button_x, button_y, button_w, button_h);
            self.display_ctx.set_fill_style_str("#e6efef");
            self.display_ctx.set_font("12px monospace");
            self.display_ctx.set_text_align("center");
            let _ =
                self.display_ctx
                    .fill_text("Teleport", button_x + button_w / 2.0, button_y + 14.0);
        }
        ...
        let _ = self
            .display_ctx
            .fill_text(help_text, map_left, map_top - 12.0);
    }
    ...
    fn draw_pixel_if_outside_mask(&self, x: i32, y: i32, mask_cx: i32, mask_cy: i32, mask_r2: i32) {
        let dx = x - mask_cx;
        let dy = y - mask_cy;
