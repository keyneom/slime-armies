...
use crate::{
    webrtc_socket::{
        message_loop, signaling_loop, MessageLoopFuture, Packet, PeerEvent,
        SignalingRequest, SocketControl, UseMessenger, UseSignaller,
    },
    Error,
};
...
impl<C: ChannelPlurality> WebRtcSocket<C> {
    ...
    pub fn try_update_peers(&mut self) -> Result<Vec<(PeerId, PeerState)>, ChannelError> {
        while let Ok(res) = self.known_peer_rx.try_next() {
            match res {
                Some((id, present)) => {
                    if present {
    ...
    pub fn set_desired_peers<I>(&mut self, peers: I)
    where
    ...
    /// Detach from signaling while keeping existing peer data channels.
    pub fn detach_signaling(&mut self) {
        let _ = self.control_tx.unbounded_send(SocketControl::DetachSignaling);
    }
...
pub struct MessageLoopChannels {
    pub requests_sender: futures_channel::mpsc::UnboundedSender<SignalingRequest>,
...
async fn run_socket(
    ...
    let (requests_sender, requests_receiver) =
        futures_channel::mpsc::unbounded::<SignalingRequest>();
